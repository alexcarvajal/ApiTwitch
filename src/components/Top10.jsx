            ...
            <div className="flex items-center  bg-custom-Bg justify-center h-[20]">
                <ul className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-16">
                    {data.slice(0, 10).map(stream => (
                        <li key={stream.id} className="group border-color-black p-4 shadow-md hover:shadow-lg transform transition-transform duration-300 hover:scale-105 hover:text-customTextHover ">
...
<ul className="">
    {data.slice(0, 10).map(stream => (
        <li key={stream.id}>
            <img src={stream.box_art_url.replace('{width}x{height}', '188x250')} alt={stream.name}/>
           {stream.name}
        </li>
    ))}
</ul> */}